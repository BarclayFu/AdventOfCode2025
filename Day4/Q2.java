...
public class Q2 {
  ...
  private static boolean check(List<List<Character>> map, int i, int j, Queue<int[]> remove) {
    int count = 0;
    if (i > 0) {
    ...
    if (count < 4 && map.get(i).get(j) == '@') {
      remove.offer(new int[] { i, j });
      return true;
    }
    return false;
  }
